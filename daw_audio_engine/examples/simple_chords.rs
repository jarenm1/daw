...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create synth - 8 voices with shorter release for clean transitions
    let mut synth = SineSynth::new(8, sample_rate);
    synth.set_gain(0.6);
    // Set shorter ADSR for cleaner chord transitions
    synth.set_adsr(0.01, 0.1, 0.6, 0.15);

    let synth = Arc::new(Mutex::new(synth));
    let synth_clone = synth.clone();

    let processor = ChordProcessor::new(synth);
    ...
    // Create clip with chords and silence between them
    let clip = create_spaced_chords();
    ...
    println!("  0-1s: C Major");
    println!("  1-2s: silence");
    println!("  2-3s: F Major");
    println!("  3-4s: silence");
    println!("  4-5s: G Major");
    println!("  5-6s: silence");
    println!("  6-7s: C Major");
    println!("  7-8s: silence\n");
    ...
    loop {
        ...
        if last_update.elapsed() >= Duration::from_millis(100) {
            ...
            if chord_str != last_chord {
                println!("\n{} Playing: {} ({} voices)", status, chord, voices);
...
fn create_spaced_chords() -> MidiClip {
    let mut clip = MidiClip::new("Chords", 0.0, 8.0);

    // C Major at 0s - plays for 1 second, silence for 1 second
    clip.add_note(MidiNote::new(60, 100, 0.0, 1.0)); // C4
    clip.add_note(MidiNote::new(64, 100, 0.0, 1.0)); // E4
    clip.add_note(MidiNote::new(67, 100, 0.0, 1.0)); // G4

    // F Major at 2s
    clip.add_note(MidiNote::new(65, 100, 2.0, 1.0)); // F4
    clip.add_note(MidiNote::new(69, 100, 2.0, 1.0)); // A4
    clip.add_note(MidiNote::new(72, 100, 2.0, 1.0)); // C5

    // G Major at 4s
    clip.add_note(MidiNote::new(67, 100, 4.0, 1.0)); // G4
    clip.add_note(MidiNote::new(71, 100, 4.0, 1.0)); // B4
    clip.add_note(MidiNote::new(74, 100, 4.0, 1.0)); // D5

    // C Major at 6s
    clip.add_note(MidiNote::new(60, 100, 6.0, 1.0)); // C4
    clip.add_note(MidiNote::new(64, 100, 6.0, 1.0)); // E4
    clip.add_note(MidiNote::new(67, 100, 6.0, 1.0)); // G4
...
impl AudioProcessor for ChordProcessor {
